...
private:
	...
	int HasCurrentProductTailIterm(vector<Triple<int, int, set<ParseTag>>>& LR1ItermList, ParseTag Tag, ParseTag CurrentTag)
	{
		vector<int> Result;
		for(auto j = 0; j < LR1ItermList.size(); j++)
		{
			auto& CurrentIterm = LR1ItermList[j];
			if(IsProductEnd(CurrentIterm.first, CurrentIterm.second) && CurrentIterm.TagMap.find(Tag) != CurrentIterm.TagMap.end() && GetSymbol(CurrentIterm.first, CurrentIterm.second - 1).Tag == CurrentTag)
			{
				return j;
			}
		}
	...
	bool ParsingRegex(vector<shared_ptr<RegexToken>>& TokenStream)
	{
		...
		for(auto i = 0; i < TokenStream.size();)
		{
			...
			if(FindIter == LR1ItemSet[GetIndex].NextStauts.end())
			{
				...
				auto ResultIndex = HasCurrentProductTailIterm(CurrentItermList, Tag, CurrentTag);
				if(ResultIndex == -1)
				{
					//出现错误字符
